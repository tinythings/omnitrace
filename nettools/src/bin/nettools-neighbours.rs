...
impl Callback<NetToolsEvent> for JsonCb {
    fn mask(&self) -> u64 {
        (NetToolsMask::NEIGHBOUR_ADDED | NetToolsMask::NEIGHBOUR_REMOVED | NetToolsMask::NEIGHBOUR_CHANGED).bits()
    }
    ...
    async fn call(&self, ev: &NetToolsEvent) -> Option<CallbackResult> {
        match ev {
            ...
            NetToolsEvent::NeighbourChanged { old, new } => {
                println!(
                    "neighbour changed: {} lladdr {} dev {} -> lladdr {} dev {}",
                    old.address, old.mac, old.iface, new.mac, new.iface
                );
